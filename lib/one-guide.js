...
var OneGuide = inherit(/** @lends OneGuide.prototype */ {
    ...
    addAdapter: function (adapterConf) {
        ...
        var configFile = this.getConfigFilename(AdapterClass.adapterName);
        var config = AdapterClass.loadConfig(configFile);
        var adapter = new AdapterClass({
            config: AdapterClass.mergeConfigs(config, this.params.override[AdapterClass.adapterName])
        });
